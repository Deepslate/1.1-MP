...
class BinaryStream{
	...
	public function getRoundedFloat(int $accuracy) : float{
		return (\round((\ENDIANNESS === 0 ? \unpack("f", $this->get(4))[1] : \unpack("f", \strrev($this->get(4)))[1]),  $accuracy));
	}
	...
	public function getRoundedLFloat(int $accuracy) : float{
		return (\round((\ENDIANNESS === 0 ? \unpack("f", \strrev($this->get(4)))[1] : \unpack("f", $this->get(4))[1]),  $accuracy));
	}
